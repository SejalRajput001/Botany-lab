...
def build_default_records(num_students, num_experiments):
    """Generate plausible fake progress records so the app isn't empty on first run."""
    records = []
    for s_idx in range(1, num_students + 1):
        # Each student has progressed a different amount through the syllabus
        progressed = random.randint(4, num_experiments)
        completed_cut = random.randint(2, progressed)
        for e_idx in range(1, num_experiments + 1):
            if e_idx <= completed_cut:
                status = "completed"
                marks = round(random.uniform(6.0, 10.0), 1)
                date = f"2026-0{random.randint(1, 6)}-{random.randint(10, 28):02d}"
                remarks = random.choice([
                    "Good technique.", "Neat lab record.", "Well labelled diagram.",
                    "Needs cleaner staining next time.", "", "",
                ])
            elif e_idx <= progressed:
                status = "in_progress"
                marks = None
                date = ""
                remarks = ""
                ...
                date = ""
                remarks = ""
            records.append((s_idx, e_idx, status, marks, date, remarks))
    return records
...
def init_db():
    with db_lock:
        ...
        if cur.fetchone()["c"] == 0:
            fake_records = build_default_records(len(DEFAULT_STUDENTS), len(DEFAULT_EXPERIMENTS))
            cur.executemany(
